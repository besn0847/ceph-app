...
monmaptool --create --add mon0 __MON_HOST__ --fsid a7f64266-0894-4f1e-a635-d0aeaca0e993 /tmp/monmap

mkdir /var/lib/ceph/mon/ceph-mon0

ceph-mon --mkfs -i mon0 --monmap /tmp/monmap --keyring /tmp/ceph.mon.keyring

echo "[program:ceph-mon]" > /etc/supervisor/conf.d/ceph-mon.conf

echo "command=ceph-mon -i mon0" >> /etc/supervisor/conf.d/ceph-mon.conf
